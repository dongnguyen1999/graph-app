...
export default class InfoPane extends Component{
    ...
    /**
     * Get data from state match with this vertex,
    ...
    prepareContent(){
        ...
        let width = height = 0; //init width and height at 0
        const { node, state } = this.props;// to get nodeId and state data
        for (let prop in state) {// loop though props of state
            if (Object.prototype.hasOwnProperty.call(state, prop)) {
                ...
                if (typeof state[prop] != "object"){//if prop is a non-object variable 
                    content.set(prop, state[prop]);//add to content
                    ...
                    content.set(key, state[prop][node.id]);
